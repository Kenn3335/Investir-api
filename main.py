...
USDT_TRON_ADDRESS = os.getenv("USDT_TRON_ADDRESS", "METE_WALLET_TRON_LA")
...
STARTER_MAX_WITHDRAW = int(os.getenv("STARTER_MAX_WITHDRAW", "200"))
...
def home():
    return """
    <html>
    <head><title>VestiCore</title></head>
    <body>
        <h1>VestiCore</h1>
        <h2>Platfòm dijital nou an</h2>
        <p>Kreye kont oswa konekte pou jwenn aksè ak espas ou.</p>
        <a href="/register">Kreye kont</a><br><br>
...
def register_page():
    ...
    <h2>Kreye kont</h2>
    <form method="post">
        <input name="username" placeholder="Non itilizatè"><br><br>
        <input type="password" name="password" placeholder="Modpas"><br><br>
        <input name="ref" placeholder="Kòd referral (opsyonèl)"><br><br>
        <button>Kreye kont</button>
    </form>
    </body>
    </html>
    """
...
def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    return f"""
    <html>
    <head><title>VestiCore Dashboard</title></head>
    <body>
    <h1>Byenveni {user.username}</h1>
    <h3>Balans</h3>
    <p>{user.balance} USDT</p>
    <h2>Sèvis nou yo</h2>
    <p>💰 Depo USDT</p>
    <p>💸 Retrè</p>
    <p>👥 Referral</p>
...
def deposit_info(request: Request, db: Session = Depends(get_db)):
    user = current_user(request, db)
    return {
        "network": "TRC20",
        "currency": "USDT",
...
def deposit(
    ...
    db.add(deposit)
    add_log(db, user.username, "Depo voye pou verifikasyon")
    db.commit()
    return {"message": "Depo resevwa", "fee": fee, "status": "pending"}
...
def withdraw(
    ...
    if amount < MIN_WITHDRAW:
        raise HTTPException(status_code=400, detail=f"Minimum retrè se {MIN_WITHDRAW} USDT")
    if amount > STARTER_MAX_WITHDRAW:
        raise HTTPException(status_code=400, detail=f"Limit retrè depase {STARTER_MAX_WITHDRAW} USDT")
    if user.balance < amount:
        raise HTTPException(status_code=400, detail="Balans pa ase")
    
    fee = amount * WITHDRAW_FEE / 100
    withdraw = Withdraw(username=user.username, amount=amount, wallet=wallet, status="pending")
    db.add(withdraw)
    add_log(db, user.username, "Demann retrè voye")
    db.commit()
    return {"message": "Retrè voye pou verifikasyon", "fee": fee, "status": "pending"}
...
def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    return f"""
    <html>
    <head><title>VestiCore Admin</title></head>
    <body>
    <h1>Admin Dashboard</h1>
    <h2>Itilizatè yo</h2><p>Total: {len(users)}</p>
    <h2>Depo</h2><p>Total demann: {len(deposits)}</p>
    <h2>Retrè</h2><p>Total demann: {len(withdraws)}</p>
    <h2>Activity Logs</h2><p>Total aktivite: {len(logs)}</p>
...
def referral_page(request: Request, db: Session = Depends(get_db)):
    ...
    return f"""
    <html>
    <head><title>Referral</title></head>
    <body>
    <h1>Referral VestiCore</h1>
    <p>Kòd pa ou: {user.referral_code}</p>
    <p>Lyen envitasyon: /register?ref={user.referral_code}</p>
    <h3>Moun ou envite: {len(referrals)}</h3>
    <a href="/dashboard">Retounen Dashboard</a>
    </body>
    </html>
    """
...
def get_plans(request: Request, db: Session = Depends(get_db)):
    ...
    plans = db.query(Plan).all()
    return [
        {"name": plan.name, "price": plan.price, "duration": plan.duration, "description": plan.description}
        for plan in plans
    ]
...
# =====================
# POU RALIWAY - SI W AP ITILIZE UVICORN DIREKT
# =====================
