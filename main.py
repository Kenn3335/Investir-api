...
# =====================
# STIL CSS - NOUVO DESIGN PI MODÈN
# =====================
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
        ...
        background: linear-gradient(135deg, #0a0e27 0%, #1a1a4e 30%, #0d1b3e 60%, #0a0e27 100%);
        min-height: 100vh;
        display: flex;
    ...
    .container {
        ...
        max-width: 1100px;
        width: 100%;
        box-shadow: 0 30px 80px rgba(0,0,0,0.5), 0 0 60px rgba(255,215,0,0.02);
        margin-top: 10px;
        transition: all 0.3s ease;
    }
    .container:hover {
        box-shadow: 0 35px 90px rgba(0,0,0,0.6), 0 0 80px rgba(255,215,0,0.03);
    }
    ...
    .logo-diamond {
        ...
        height: 65px;
        margin-bottom: 6px;
        filter: drop-shadow(0 0 35px rgba(255,215,0,0.3));
        animation: floatGlow 3s ease-in-out infinite;
    }
    .logo-diamond svg { width: 100%; height: 100%; }
    @keyframes floatGlow {
        0%, 100% { transform: translateY(0px); filter: drop-shadow(0 0 35px rgba(255,215,0,0.3)); }
        50% { transform: translateY(-6px); filter: drop-shadow(0 0 55px rgba(255,215,0,0.5)); }
    }
    .logo h1 {
        color: #ffffff;
        font-size: 26px;
    ...
    .logo h1 span { color: #ffd700; text-shadow: 0 0 40px rgba(255,215,0,0.25); }
    .logo p {
    ...
    .lang-selector a:hover { color: #ffd700; background: rgba(255,215,0,0.06); border-color: rgba(255,215,0,0.05); }
    .lang-selector a.active { color: #ffd700; background: rgba(255,215,0,0.08); border-color: rgba(255,215,0,0.08); }
    ...
    .header-left .mini-logo svg { width: 100%; height: 100%; }
    ...
    .header-left .app-name span { color: #ffd700; }
    ...
    .notif-bell:hover { color: rgba(255,255,255,0.6); }
    .notif-bell .badge {
        position: absolute;
        top: -5px;
        right: -5px;
    ...
    .dash-stat.gold { border-color: rgba(255,215,0,0.06); }
    .dash-stat.green { border-color: rgba(74,222,128,0.06); }
    .dash-stat.blue { border-color: rgba(59,130,246,0.06); }
    .dash-stat.purple { border-color: rgba(168,85,247,0.06); }
    .dash-stat.red { border-color: rgba(255,107,107,0.06); }
    ...
    .modal-plan-card h4 { color: #ffd700; font-size: 14px; font-weight: 600; }
    .modal-plan-card p { color: rgba(255,255,255,0.35); font-size: 11px; margin: 2px 0; }
    ...
    .modal-plan-card .plan-price small { color: rgba(255,255,255,0.25); font-size: 10px; font-weight: 400; }
    ...
    .modal-plan-card .btn-sm:hover { transform: scale(1.05); box-shadow: 0 4px 20px rgba(255,215,0,0.2); }
    ...
    .simple-modal-content .btn:hover { transform: scale(1.05); box-shadow: 0 4px 25px rgba(255,215,0,0.25); }
    ...
    .section h4 span { color: rgba(255,255,255,0.15); font-size: 11px; font-weight: 400; }
    ...
    @media (max-width: 600px) {
        ...
        .modal-plan-card .plan-price { text-align: center; margin-top: 6px; }
    }
</style>
"""
    ...
    <body>
        <div class="container" style="max-width:600px;">
            ...
            <div style="text-align:center;margin:15px 0 25px;">
                <p style="color:rgba(255,255,255,0.5);font-size:16px;line-height:1.8;">
            ...
            <a href="/register" class="btn" style="display:block;text-align:center;text-decoration:none;width:100%;padding:15px;font-size:16px;">{LANG[lang].get('home_btn_register', 'Create account')}</a>
            <br>
            <a href="/login" class="btn btn-secondary" style="display:block;text-align:center;text-decoration:none;width:100%;padding:15px;font-size:16px;">{LANG[lang].get('home_btn_login', 'Login')}</a>
...
def register_page(request: Request, ref: str = None, error: str = None):
    ...
    error_html = f'<div class="error-message" style="background:rgba(255,107,107,0.06);border:1px solid rgba(255,107,107,0.08);color:#ff6b6b;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:15px;text-align:center;">❌ {error}</div>' if error else ''
    return f"""
    <html>
    <head>
    ...
    <body>
        <div class="container" style="max-width:480px;">
            ...
            <div style="color:rgba(255,255,255,0.3);font-size:13px;text-align:center;margin-bottom:20px;">{LANG[lang].get('register_sub', 'Start your investment journey')}</div>
            
            {error_html}
            
            <form method="post" action="/register">
                <div class="form-group" style="margin-bottom:14px;">
                    <label style="color:rgba(255,255,255,0.4);font-size:12px;font-weight:500;display:block;margin-bottom:4px;">{LANG[lang].get('register_username', 'Username')}</label>
                    <input type="text" name="username" placeholder="{LANG[lang].get('register_username', 'Username')}" style="width:100%;padding:12px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;transition:all 0.3s;" required>
                </div>
                <div class="form-group" style="margin-bottom:14px;">
                    <label style="color:rgba(255,255,255,0.4);font-size:12px;font-weight:500;display:block;margin-bottom:4px;">{LANG[lang].get('register_password', 'Password')}</label>
                    <input type="password" name="password" placeholder="{LANG[lang].get('register_password', 'Password')}" style="width:100%;padding:12px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;transition:all 0.3s;" required>
                </div>
                <div class="form-group" style="margin-bottom:18px;">
                    <label style="color:rgba(255,255,255,0.4);font-size:12px;font-weight:500;display:block;margin-bottom:4px;">{LANG[lang].get('register_ref', 'Referral code (optional)')}</label>
                    <input type="text" name="ref" placeholder="{LANG[lang].get('register_ref', 'Referral code (optional)')}" {ref_value} style="width:100%;padding:12px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;transition:all 0.3s;">
                </div>
                <button type="submit" class="btn" style="width:100%;padding:14px;font-size:15px;">{LANG[lang].get('register_btn', 'Create account')}</button>
            </form>
            
            <div style="text-align:center;margin-top:16px;color:rgba(255,255,255,0.3);font-size:13px;">
...
def login_page(request: Request, error: str = None, success: str = None):
    lang = get_lang(request)
    error_html = f'<div class="error-message" style="background:rgba(255,107,107,0.06);border:1px solid rgba(255,107,107,0.08);color:#ff6b6b;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:15px;text-align:center;">❌ {error}</div>' if error else ''
    success_html = f'<div style="background:rgba(74,222,128,0.06);border:1px solid rgba(74,222,128,0.08);color:#4ade80;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:15px;text-align:center;">✅ {success}</div>' if success else ''
    return f"""
    <html>
    <head>
    ...
    <body>
        <div class="container" style="max-width:480px;">
            ...
            <div style="color:rgba(255,255,255,0.3);font-size:13px;text-align:center;margin-bottom:20px;">{LANG[lang].get('login_sub', 'Login to access your space')}</div>
            ...
            <form method="post" action="/login">
                ...
                <button type="submit" class="btn" style="width:100%;padding:14px;font-size:15px;">{LANG[lang].get('login_btn', 'Login')}</button>
            </form>
...
def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    logs = db.query(ActivityLog).filter(ActivityLog.username == user.username).order_by(ActivityLog.date.desc()).limit(5).all()
    ...
    admin_link = f'<a href="/admin" style="color:rgba(255,255,255,0.04);text-decoration:none;font-size:9px;">Admin</a>' if user.is_admin == 1 else ''
        ...
        <div class="container">
            ...
            <div class="header">
                <div class="header-left">
                    ...
                    <div class="app-name">Vesti<span>Core</span></div>
                </div>
            ...
            <div class="sections">
                <div class="section">
                    <h4>📜 {LANG[lang].get('recent_transactions', 'Recent Transactions')}</h4>
                    {''.join([f'<div class="tx-item"><span class="left">{log.date.strftime("%d/%m %H:%M")} - {log.action[:25]}...</span><span class="right positive">💰</span></div>' for log in logs[:5]]) if logs else '<div style="color:rgba(255,255,255,0.12);text-align:center;padding:12px 0;font-size:12px;">Aucune transaction</div>'}
                </div>
                <div class="section">
                    <h4>📈 {LANG[lang].get('earnings_history', 'Earnings History')}</h4>
                    {''.join([f'<div class="tx-item"><span class="left">{log.date.strftime("%d/%m %H:%M")}</span><span class="right positive">+{log.action.split(":")[-1].strip() if ":" in log.action else "0.00"} USDT</span></div>' for log in logs[:5] if "Benefis" in log.action]) if logs else '<div style="color:rgba(255,255,255,0.12);text-align:center;padding:12px 0;font-size:12px;">Aucun gain</div>'}
                </div>
            </div>
            
            <!-- PROFILE SECTION -->
            <div id="profileSection" class="section" style="margin-top:6px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
                    <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:8px;">
                        <div style="color:rgba(255,255,255,0.25);font-size:9px;text-transform:uppercase;">Nom</div>
                        <div style="color:#ffffff;font-size:14px;font-weight:600;">{user.username}</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:8px;">
                        <div style="color:rgba(255,255,255,0.25);font-size:9px;text-transform:uppercase;">ID</div>
                        <div style="color:#ffd700;font-size:14px;font-weight:600;">{user.user_id_display or 'VC000001'}</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:8px;">
                        <div style="color:rgba(255,255,255,0.25);font-size:9px;text-transform:uppercase;">KYC</div>
                        <div style="color:#ffffff;font-size:14px;font-weight:600;">{kyc_status_text}</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:8px;">
                        <div style="color:rgba(255,255,255,0.25);font-size:9px;text-transform:uppercase;">Plan</div>
                        <div style="color:#4ade80;font-size:14px;font-weight:600;">{active_plan_name}</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:8px;">
                        <div style="color:rgba(255,255,255,0.25);font-size:9px;text-transform:uppercase;">Solde</div>
                        <div style="color:#ffd700;font-size:14px;font-weight:600;">{user.balance:.2f} USDT</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:8px;">
                        <div style="color:rgba(255,255,255,0.25);font-size:9px;text-transform:uppercase;">Membre depuis</div>
                        <div style="color:#ffffff;font-size:14px;font-weight:600;">{user.created_at.strftime('%d/%m/%Y')}</div>
                    </div>
                </div>
                <div style="margin-top:10px;">
                    <a href="/kyc" class="btn btn-secondary" style="padding:6px 16px;font-size:11px;border-radius:6px;text-decoration:none;display:inline-block;">🪪 Gérer KYC</a>
                </div>
            </div>
            
            <!-- FOOTER -->
            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;padding-top:10px;border-top:1px solid rgba(255,255,255,0.02);">
                {admin_link}
                <span style="color:rgba(255,255,255,0.04);font-size:8px;letter-spacing:2px;">{LANG[lang].get('footer', '© 2026 VestiCore. All rights reserved.')}</span>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-plan-card">
                    <div>
                        ...
                        <p style="color:rgba(255,255,255,0.2);font-size:10px;">💵 {plan.price * (plan.daily_return / 100):.2f} USDT/jour • 💎 Total: {plan.price * (plan.daily_return / 100) * plan.duration:.2f} USDT</p>
                    </div>
                    <div class="plan-price">
                        {plan.price} <small>USDT</small>
                        <br>
                        <button onclick="acheterPlan({plan.id}); closePlansModal();" class="btn-sm">Acheter</button>
                    </div>
                </div>
        ...
        <div class="container" style="max-width:600px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <div style="display:flex;align-items:center;gap:12px;">
                    <div class="logo-diamond" style="width:40px;height:40px;">{LOGO_SVG}</div>
                    <div><div style="color:#ffffff;font-size:20px;font-weight:700;">Vesti<span style="color:#ffd700;">Core</span></div><div style="color:rgba(255,255,255,0.15);font-size:8px;letter-spacing:3px;">KYC</div></div>
                </div>
                <a href="/dashboard" style="color:rgba(255,255,255,0.2);text-decoration:none;font-size:12px;">← Retour</a>
            </div>
            
            <div style="color:#ffffff;font-size:22px;font-weight:600;text-align:center;">🪪 Vérification KYC</div>
            <div style="color:rgba(255,255,255,0.3);font-size:13px;text-align:center;margin-bottom:20px;">Téléchargez vos documents pour vérifier votre identité</div>
            
            <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:20px;border:1px solid rgba(255,255,255,0.04);">
                <div style="margin-bottom:15px;">
                    <label style="color:rgba(255,255,255,0.3);font-size:11px;display:block;margin-bottom:4px;">Status actuel</label>
                    <div style="padding:10px;border-radius:8px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);">
                ...
                <form method="post" action="/kyc/submit">
                    <div class="form-group" style="margin-bottom:12px;">
                        <label style="color:rgba(255,255,255,0.3);font-size:11px;display:block;margin-bottom:4px;">Nom complet</label>
                        <input type="text" name="full_name" value="{user.full_name or ''}" placeholder="Votre nom complet" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;">
                    </div>
                    <div class="form-group" style="margin-bottom:12px;">
                        <label style="color:rgba(255,255,255,0.3);font-size:11px;display:block;margin-bottom:4px;">Email</label>
                        <input type="email" name="email" value="{user.email or ''}" placeholder="votre@email.com" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;">
                    </div>
                    <div class="form-group" style="margin-bottom:12px;">
                        <label style="color:rgba(255,255,255,0.3);font-size:11px;display:block;margin-bottom:4px;">Téléphone</label>
                        <input type="text" name="phone" value="{user.phone or ''}" placeholder="+509 0000 0000" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;">
                    </div>
                    <div class="form-group" style="margin-bottom:16px;">
                        <label style="color:rgba(255,255,255,0.3);font-size:11px;display:block;margin-bottom:4px;">Pays</label>
                        <input type="text" name="country" value="{user.country or ''}" placeholder="Haïti" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;">
                    </div>
                    <button type="submit" class="btn" style="width:100%;padding:12px;font-size:14px;">Soumettre KYC</button>
                </form>
            </div>
        </div>
        {get_modal_html()}
    ...
    <body>
        <div class="container" style="max-width:550px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <div style="display:flex;align-items:center;gap:12px;">
                    <div class="logo-diamond" style="width:40px;height:40px;">{LOGO_SVG}</div>
                    <div><div style="color:#ffffff;font-size:20px;font-weight:700;">Vesti<span style="color:#ffd700;">Core</span></div><div style="color:rgba(255,255,255,0.15);font-size:8px;letter-spacing:3px;">FINANCE & REWARDS</div></div>
                </div>
                <a href="/dashboard" style="color:rgba(255,255,255,0.2);text-decoration:none;font-size:12px;">← {LANG[lang].get('deposit_back', 'Back')}</a>
            </div>
            
            <div style="color:#ffffff;font-size:22px;font-weight:600;text-align:center;">{LANG[lang].get('deposit_title', '💰 USDT Deposit')}</div>
            
            <div style="background:rgba(255,0,0,0.04);border:1px solid rgba(255,0,0,0.06);border-radius:10px;padding:10px;margin-bottom:14px;">
            ...
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:14px;border:1px solid rgba(255,255,255,0.04);margin-bottom:14px;">
                <p style="color:rgba(255,255,255,0.25);font-size:9px;">{LANG[lang].get('deposit_network', '🌐 NETWORK')}</p>
                <p style="color:#4ade80;font-weight:700;font-size:14px;">TRC20 (Tron)</p>
                
                <p style="color:rgba(255,255,255,0.25);font-size:9px;margin-top:6px;">{LANG[lang].get('deposit_address', '🏦 ADDRESS')}</p>
                <p style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px;color:#ffd700;font-size:11px;word-break:break-all;font-family:monospace;">{USDT_TRON_ADDRESS}</p>
                
                <p style="color:rgba(255,255,255,0.25);font-size:9px;margin-top:6px;">{LANG[lang].get('deposit_contract', '📄 CONTRACT')}</p>
                <p style="background:rgba(0,0,0,0.3);padding:6px;border-radius:6px;color:rgba(255,255,255,0.3);font-size:10px;word-break:break-all;font-family:monospace;">{USDT_CONTRACT_ADDRESS}</p>
                
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px;">
                    <div><p style="color:rgba(255,255,255,0.2);font-size:8px;">{LANG[lang].get('deposit_fee', '💸 FEE')}</p><p style="color:#ffffff;font-size:13px;">{DEPOSIT_FEE}%</p></div>
                    <div><p style="color:rgba(255,255,255,0.2);font-size:8px;">{LANG[lang].get('deposit_time', '⏱️ TIME')}</p><p style="color:#ffffff;font-size:13px;">~1-2 min</p></div>
                </div>
            </div>
            
            <h3 style="color:rgba(255,255,255,0.5);font-size:13px;margin-bottom:8px;">{LANG[lang].get('deposit_verify', 'Verify your deposit')}</h3>
            <form method="post" action="/deposit">
                <div class="form-group" style="margin-bottom:10px;">
                    <input type="number" name="amount" placeholder="{LANG[lang].get('deposit_amount', 'USDT Amount')}" step="0.01" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;" required>
                </div>
                <div class="form-group" style="margin-bottom:14px;">
                    <input type="text" name="txid" placeholder="{LANG[lang].get('deposit_txid', 'Transaction ID (txid)')}" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;" required>
                </div>
                <button type="submit" class="btn" style="width:100%;padding:12px;font-size:14px;">{LANG[lang].get('deposit_btn', 'Submit deposit')}</button>
            </form>
        </div>
        {get_modal_html()}
    </body>
...
def info_page(request: Request, db: Session = Depends(get_db)):
    ...
    for plan in plans:
        ...
        plan_html += f"""
        <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:8px 12px;margin:4px 0;border:1px solid rgba(255,255,255,0.03);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">
            <div><span style="color:#ffd700;font-size:12px;font-weight:600;">{plan.name}</span><span style="color:rgba(255,255,255,0.2);font-size:10px;margin-left:6px;">{plan.duration}j</span></div>
            <div><span style="color:rgba(255,255,255,0.3);font-size:11px;">{plan.daily_return}%/j</span><span style="color:#ffffff;font-size:13px;font-weight:700;margin-left:10px;">{plan.price} USDT</span><span style="color:#4ade80;font-size:10px;margin-left:6px;">+{total_return:.2f}</span></div>
        </div>
        """
        ...
        <div class="container" style="max-width:700px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div class="logo-diamond" style="width:36px;height:36px;">{LOGO_SVG}</div>
                    <div><div style="color:#ffffff;font-size:18px;font-weight:700;">Vesti<span style="color:#ffd700;">Core</span></div><div style="color:rgba(255,255,255,0.12);font-size:7px;letter-spacing:3px;">INFO</div></div>
                </div>
                <a href="/dashboard" style="color:rgba(255,255,255,0.2);text-decoration:none;font-size:12px;">← {LANG[lang].get('info_back', 'Back to Dashboard')}</a>
            </div>
            
            <div style="text-align:center;margin-bottom:18px;">
                <div style="font-size:32px;margin-bottom:4px;">ℹ️</div>
                <h1 style="color:#ffffff;font-size:24px;font-weight:700;">{LANG[lang].get('info_title', 'About VestiCore')}</h1>
                <p style="color:rgba(255,255,255,0.3);font-size:13px;">{LANG[lang].get('info_subtitle', 'Everything you need to know')}</p>
            </div>
            
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:14px;margin-bottom:8px;border:1px solid rgba(255,215,0,0.04);">
                <h3 style="color:#ffd700;font-size:14px;margin-bottom:4px;">💎 {LANG[lang].get('info_section1_title', 'What is VestiCore?')}</h3>
                <p style="color:rgba(255,255,255,0.5);font-size:13px;line-height:1.6;">{LANG[lang].get('info_section1_text', 'VestiCore is a digital finance platform that allows users to invest in rewarding plans and earn daily profits in USDT.')}</p>
            </div>
            
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:14px;margin-bottom:8px;border:1px solid rgba(255,215,0,0.04);">
                <h3 style="color:#ffd700;font-size:14px;margin-bottom:4px;">⚙️ {LANG[lang].get('info_section2_title', 'How does it work?')}</h3>
                <div style="color:rgba(255,255,255,0.5);font-size:13px;line-height:2;">
            ...
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:14px;margin-bottom:8px;border:1px solid rgba(255,215,0,0.04);">
                <h3 style="color:#ffd700;font-size:14px;margin-bottom:4px;">📊 {LANG[lang].get('info_section5_title', 'Investment Plans')}</h3>
                {plan_html}
            </div>
            
            <div style="background:rgba(74,222,128,0.02);border-radius:10px;padding:14px;margin-bottom:8px;border:1px solid rgba(74,222,128,0.04);">
                <h3 style="color:#4ade80;font-size:14px;margin-bottom:4px;">🔒 {LANG[lang].get('info_section3_title', 'Security')}</h3>
                <p style="color:rgba(255,255,255,0.5);font-size:13px;line-height:1.6;">{LANG[lang].get('info_section3_text', 'VestiCore uses advanced technologies to secure your data and transactions: SSL encryption, secure authentication, KYC, and manual approval of withdrawals by the administrator.')}</p>
            </div>
            
            <div style="background:rgba(255,215,0,0.02);border-radius:10px;padding:14px;margin-bottom:8px;border:1px solid rgba(255,215,0,0.04);">
                <h3 style="color:#ffd700;font-size:14px;margin-bottom:4px;">🎁 {LANG[lang].get('info_section4_title', 'Referral Program')}</h3>
                <p style="color:rgba(255,255,255,0.5);font-size:13px;line-height:1.6;">{LANG[lang].get('info_section4_text', 'Invite your friends and earn 2% bonus on the deposits of your first 10 qualified investors.')}</p>
            </div>
            
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:14px;border:1px solid rgba(255,255,255,0.03);">
                <h3 style="color:#ffffff;font-size:14px;margin-bottom:4px;">❓ FAQ</h3>
                <div style="color:rgba(255,255,255,0.4);font-size:12px;line-height:2;">
                    <div><strong style="color:rgba(255,255,255,0.6);">Minimum dépôt ?</strong> 10 USDT</div>
                    <div><strong style="color:rgba(255,255,255,0.6);">Retrait ?</strong> Après approbation admin</div>
                    <div><strong style="color:rgba(255,255,255,0.6);">Frais ?</strong> 3% dépôt, 5% retrait</div>
                </div>
            </div>
...
def settings_page(request: Request, db: Session = Depends(get_db), error: str = None, success: str = None):
    ...
    error_html = f'<div style="background:rgba(255,107,107,0.06);border:1px solid rgba(255,107,107,0.08);color:#ff6b6b;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:14px;text-align:center;">❌ {error}</div>' if error else ''
    success_html = f'<div style="background:rgba(74,222,128,0.06);border:1px solid rgba(74,222,128,0.08);color:#4ade80;padding:10px 14px;border-radius:8px;font-size:13px;margin-bottom:14px;text-align:center;">✅ {success}</div>' if success else ''
    
    deposits = db.query(Deposit).filter(Deposit.username == user.username).order_by(Deposit.date.desc()).limit(10).all()
    withdraws = db.query(Withdraw).filter(Withdraw.username == user.username).order_by(Withdraw.date.desc()).limit(10).all()
    earnings = db.query(ActivityLog).filter(ActivityLog.username == user.username, ActivityLog.action.like("Benefis%")).order_by(ActivityLog.date.desc()).limit(10).all()
    
    tx_html = ""
    for d in deposits:
        tx_html += f'<div class="tx-item"><span class="left">💰 Dépôt • {d.date.strftime("%d/%m %H:%M")}</span><span class="right positive">+{d.net_amount:.2f} USDT</span></div>'
    for w in withdraws:
        tx_html += f'<div class="tx-item"><span class="left">💸 Retrait • {w.date.strftime("%d/%m %H:%M")}</span><span class="right negative">-{w.amount:.2f} USDT</span></div>'
    if not tx_html:
        tx_html = '<div style="color:rgba(255,255,255,0.12);text-align:center;padding:12px 0;font-size:12px;">Aucune transaction</div>'
    
    earnings_html = ""
    for e in earnings:
        amount = e.action.split(":")[-1].strip() if ":" in e.action else "0.00"
        earnings_html += f'<div class="tx-item"><span class="left">📈 {e.date.strftime("%d/%m %H:%M")}</span><span class="right positive">+{amount} USDT</span></div>'
    if not earnings_html:
        earnings_html = '<div style="color:rgba(255,255,255,0.12);text-align:center;padding:12px 0;font-size:12px;">Aucun gain</div>'
    ...
    <body>
        <div class="container" style="max-width:700px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div class="logo-diamond" style="width:36px;height:36px;">{LOGO_SVG}</div>
                    <div><div style="color:#ffffff;font-size:18px;font-weight:700;">Vesti<span style="color:#ffd700;">Core</span></div><div style="color:rgba(255,255,255,0.12);font-size:7px;letter-spacing:3px;">SETTINGS</div></div>
                </div>
                <a href="/dashboard" style="color:rgba(255,255,255,0.2);text-decoration:none;font-size:12px;">← Retour</a>
            </div>
            
            <div style="color:#ffffff;font-size:22px;font-weight:600;text-align:center;margin-bottom:16px;">⚙️ {LANG[lang].get('settings', 'Settings')}</div>
            
            {success_html}{error_html}
            
            <!-- Changer modpas -->
            <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:18px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.03);">
                <h3 style="color:#ffd700;font-size:15px;margin-bottom:10px;">🔐 {LANG[lang].get('change_password', 'Change Password')}</h3>
                <form method="post" action="/settings/change-password">
                    <div style="margin-bottom:8px;">
                        <input type="password" name="current_password" placeholder="{LANG[lang].get('current_password', 'Current password')}" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;" required>
                    </div>
                    <div style="margin-bottom:8px;">
                        <input type="password" name="new_password" placeholder="{LANG[lang].get('new_password', 'New password')}" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;" required>
                    </div>
                    <div style="margin-bottom:10px;">
                        <input type="password" name="confirm_password" placeholder="{LANG[lang].get('confirm_password', 'Confirm password')}" style="width:100%;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:8px;color:#ffffff;font-size:14px;outline:none;" required>
                    </div>
                    <button type="submit" class="btn btn-secondary" style="width:100%;padding:10px;font-size:13px;">{LANG[lang].get('change_password', 'Change password')}</button>
                </form>
            </div>
            
            <!-- Transactions -->
            <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:18px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.03);">
                <h3 style="color:#4ade80;font-size:15px;margin-bottom:10px;">📊 {LANG[lang].get('transactions_history', 'Transactions History')}</h3>
                <div style="max-height:200px;overflow-y:auto;">{tx_html}</div>
            </div>
            
            <!-- Gains -->
            <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:18px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.03);">
                <h3 style="color:#ffd700;font-size:15px;margin-bottom:10px;">📈 {LANG[lang].get('earnings_history', 'Earnings History')}</h3>
                <div style="max-height:200px;overflow-y:auto;">{earnings_html}</div>
            </div>
...
def referral_page(request: Request, db: Session = Depends(get_db)):
    ...
    status_text = "✅ Bonus 2% débloqué!" if qualified_investors >= REFERRAL_MIN_INVESTORS else LANG[lang].get('referral_needed', '{needed} more investors for 2%').format(needed=needed)
    ...
    <body>
        <div class="container" style="max-width:520px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div class="logo-diamond" style="width:36px;height:36px;">{LOGO_SVG}</div>
                    <div><div style="color:#ffffff;font-size:18px;font-weight:700;">Vesti<span style="color:#ffd700;">Core</span></div><div style="color:rgba(255,255,255,0.12);font-size:7px;letter-spacing:3px;">REFERRAL</div></div>
                </div>
                <a href="/dashboard" style="color:rgba(255,255,255,0.2);text-decoration:none;font-size:12px;">← {LANG[lang].get('referral_back', 'Back')}</a>
            </div>
            
            <div style="color:#ffffff;font-size:22px;font-weight:600;text-align:center;">👥 {LANG[lang].get('referral_title', 'Referral')}</div>
            
            <div style="background:rgba(255,215,0,0.04);border-radius:10px;padding:14px;border:1px solid rgba(255,215,0,0.04);margin-top:12px;">
                <p style="color:rgba(255,255,255,0.25);font-size:9px;">{LANG[lang].get('referral_code', 'YOUR CODE')}</p>
                <p style="color:#ffd700;font-size:20px;font-weight:700;font-family:monospace;">{user.referral_code}</p>
            </div>
            
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:10px;margin-top:10px;">
                <p style="color:rgba(255,255,255,0.25);font-size:9px;">{LANG[lang].get('referral_link', 'INVITATION LINK')}</p>
                <p style="background:rgba(0,0,0,0.3);padding:6px;border-radius:6px;color:rgba(255,255,255,0.4);font-size:11px;word-break:break-all;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px;">
                <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;text-align:center;">
                    <p style="color:rgba(255,255,255,0.2);font-size:9px;">{LANG[lang].get('referral_count', 'People invited')}</p>
                    <p style="color:#ffffff;font-size:20px;font-weight:700;">{len(referrals)}</p>
                </div>
                <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;text-align:center;">
                    <p style="color:rgba(255,255,255,0.2);font-size:9px;">{LANG[lang].get('referral_investors', 'Qualified investors')}</p>
                    <p style="color:#4ade80;font-size:20px;font-weight:700;">{qualified_investors}/{REFERRAL_MIN_INVESTORS}</p>
                </div>
            </div>
            
            <div style="background:rgba(255,215,0,0.03);border-radius:8px;padding:10px;margin-top:10px;text-align:center;border:1px solid rgba(255,215,0,0.04);">
                <p style="color:rgba(255,255,255,0.4);font-size:12px;">{status_text}</p>
                <p style="color:rgba(255,255,255,0.2);font-size:10px;margin-top:2px;">{LANG[lang].get('referral_bonus', 'Bonus received')}: <span style="color:#ffd700;font-weight:700;">{total_bonus:.2f} USDT</span></p>
            </div>
        </div>
        {get_modal_html()}
    </body>
...
def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    for d in pending_deposits:
        deposit_html += f"""
        <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;margin:4px 0;border:1px solid rgba(255,255,255,0.03);">
            <p style="color:#ffffff;font-size:12px;"><strong>{d.username}</strong> - {d.amount} USDT</p>
            <p style="color:rgba(255,255,255,0.2);font-size:9px;">TXID: {d.txid}</p>
            <div style="display:flex;gap:6px;margin-top:4px;">
                <form method="post" action="/admin/approve-deposit/{d.id}" style="display:inline;">
                    <button style="background:#4ade80;border:none;padding:4px 12px;border-radius:5px;color:#0a0e27;font-weight:700;cursor:pointer;font-size:10px;">{LANG[lang].get('admin_approve', 'Approve')}</button>
                </form>
                <form method="post" action="/admin/reject-deposit/{d.id}" style="display:inline;">
                    <button style="background:#ff6b6b;border:none;padding:4px 12px;border-radius:5px;color:#ffffff;font-weight:700;cursor:pointer;font-size:10px;">{LANG[lang].get('admin_reject', 'Reject')}</button>
                </form>
            </div>
        </div>
        """
    ...
    for w in pending_withdraws:
        withdraw_html += f"""
        <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;margin:4px 0;border:1px solid rgba(255,255,255,0.03);">
            <p style="color:#ffffff;font-size:12px;"><strong>{w.username}</strong> - {w.amount} USDT</p>
            <p style="color:rgba(255,255,255,0.2);font-size:9px;">Wallet: {w.wallet}</p>
            <div style="display:flex;gap:6px;margin-top:4px;">
    ...
    for k in pending_kyc:
        kyc_html += f"""
        <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;margin:4px 0;border:1px solid rgba(255,255,255,0.03);">
            <p style="color:#ffffff;font-size:12px;"><strong>{k.username}</strong> - KYC en attente</p>
            <div style="display:flex;gap:6px;margin-top:4px;">
                <form method="post" action="/admin/approve-kyc/{k.id}" style="display:inline;">
                    <button style="background:#4ade80;border:none;padding:4px 12px;border-radius:5px;color:#0a0e27;font-weight:700;cursor:pointer;font-size:10px;">✅ Approuver</button>
                </form>
                <form method="post" action="/admin/reject-kyc/{k.id}" style="display:inline;">
                    <button style="background:#ff6b6b;border:none;padding:4px 12px;border-radius:5px;color:#ffffff;font-weight:700;cursor:pointer;font-size:10px;">❌ Rejeter</button>
                </form>
            </div>
        </div>
        """
    ...
    for t in tickets[:5]:
        tickets_html += f"""
        <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:8px 10px;margin:4px 0;border:1px solid rgba(255,255,255,0.03);">
            <div>
                <span style="color:#ffffff;font-size:12px;font-weight:600;">{t.subject}</span>
                <span style="font-size:9px;font-weight:600;padding:2px 8px;border-radius:6px;display:inline-block;background:rgba(74,222,128,0.1);color:#4ade80;">{t.status.upper()}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-top:2px;">
                <span style="color:rgba(255,255,255,0.2);font-size:9px;">{t.username}</span>
                <span style="color:rgba(255,255,255,0.15);font-size:9px;">{t.created_at.strftime('%d/%m %H:%M')}</span>
            </div>
        </div>
        """
    ...
    <body>
        <div class="container" style="max-width:680px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <div class="logo-diamond" style="width:36px;height:36px;">{LOGO_SVG}</div>
                    <div><div style="color:#ffffff;font-size:18px;font-weight:700;">Vesti<span style="color:#ffd700;">Core</span></div><div style="color:rgba(255,255,255,0.12);font-size:7px;letter-spacing:3px;">ADMIN</div></div>
                </div>
                <a href="/dashboard" style="color:rgba(255,255,255,0.2);text-decoration:none;font-size:12px;">{LANG[lang].get('admin_back', '← Back')}</a>
            </div>
            
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin-bottom:12px;">
                <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;text-align:center;">
                    <p style="color:rgba(255,255,255,0.2);font-size:8px;">USERS</p>
                    <p style="color:#ffffff;font-size:18px;font-weight:700;">{len(users)}</p>
                </div>
                <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;text-align:center;">
                    <p style="color:rgba(255,255,255,0.2);font-size:8px;">DEPOSITS</p>
                    <p style="color:#4ade80;font-size:18px;font-weight:700;">{len(deposits)}</p>
                </div>
                <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;text-align:center;">
                    <p style="color:rgba(255,255,255,0.2);font-size:8px;">WITHDRAWS</p>
                    <p style="color:#fbbf24;font-size:18px;font-weight:700;">{len(withdraws)}</p>
                </div>
                <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:10px;text-align:center;">
                    <p style="color:rgba(255,255,255,0.2);font-size:8px;">TICKETS</p>
                    <p style="color:#a855f7;font-size:18px;font-weight:700;">{len(tickets)}</p>
                </div>
            </div>
            
            <h3 style="color:#ffd700;font-size:13px;">📋 KYC</h3>
            {kyc_html if kyc_html else f'<p style="color:rgba(255,255,255,0.15);font-size:11px;">Aucune demande KYC</p>'}
            
            <h3 style="color:#ffd700;font-size:13px;margin-top:10px;">⏳ Dépôts</h3>
            {deposit_html if deposit_html else f'<p style="color:rgba(255,255,255,0.15);font-size:11px;">{no_pending}</p>'}
            
            <h3 style="color:#ffd700;font-size:13px;margin-top:10px;">⏳ Retraits</h3>
            {withdraw_html if withdraw_html else f'<p style="color:rgba(255,255,255,0.15);font-size:11px;">{no_pending}</p>'}
            
            <h3 style="color:#ffd700;font-size:13px;margin-top:10px;">🎫 Tickets</h3>
            {tickets_html if tickets_html else f'<p style="color:rgba(255,255,255,0.15);font-size:11px;">Aucun ticket</p>'}
            
            <div style="margin-top:10px;border-top:1px solid rgba(255,255,255,0.02);padding-top:8px;">
                <p style="color:rgba(255,255,255,0.1);font-size:8px;">{LANG[lang].get('admin_recent', 'Recent activities')}</p>
                <ul style="list-style:none;padding:0;max-height:80px;overflow-y:auto;">
                {''.join([f"<li style='color:rgba(255,255,255,0.12);font-size:9px;padding:2px 0;border-bottom:1px solid rgba(255,255,255,0.01);'>{log.date.strftime('%d/%m %H:%M')} - {log.username}: {log.action}</li>" for log in logs[-8:]])}
                </ul>
            </div>
        </div>
        {get_modal_html()}
            ...
            if referral:
                ...
                if referrer:
                    ...
                    if qualified >= REFERRAL_MIN_INVESTORS and not referrer.referral_qualified:
                        ...
                        total_invested = sum(
                            sum(d.net_amount for d in db.query(Deposit).filter(
...
def admin_backup(request: Request, db: Session = Depends(get_db)):
    ...
    data = {
        "users": [{"id": u.id, "username": u.username, "balance": u.balance, "is_admin": u.is_admin, "referral_code": u.referral_code, "kyc_status": u.kyc_status, "created_at": u.created_at.isoformat()} for u in db.query(User).all()],
        "plans": [{"id": p.id, "name": p.name, "price": p.price, "duration": p.duration, "daily_return": p.daily_return} for p in db.query(Plan).all()],
        "deposits": [{"id": d.id, "username": d.username, "amount": d.amount, "fee": d.fee, "net_amount": d.net_amount, "txid": d.txid, "status": d.status, "date": d.date.isoformat()} for d in db.query(Deposit).all()],
        "withdraws": [{"id": w.id, "username": w.username, "amount": w.amount, "fee": w.fee, "net_amount": w.net_amount, "wallet": w.wallet, "status": w.status, "date": w.date.isoformat()} for w in db.query(Withdraw).all()],
        "referrals": [{"id": r.id, "referrer": r.referrer, "invited_user": r.invited_user, "has_invested": r.has_invested, "bonus_amount": r.bonus_amount} for r in db.query(Referral).all()],
        "logs": [{"id": l.id, "username": l.username, "action": l.action, "date": l.date.isoformat()} for l in db.query(ActivityLog).all()]
    }
